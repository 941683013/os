...
void mkdir(char * dirname)/*mkdir*/
{
	...
	if(dirid!=0)
	{
		...
		if(inode->di_mode&DIDIR)
			printf("\r%s directory already existed!\n",dirname);
		else
			printf("\r%s is a filename,&can't create a dir the same name\n",dirname);
		iput(inode);
		return;
	}
...
void chdir(char * dirname)/*chdir*/
{
	...
	{
		printf("\nhas not accsee to the directory %s\n",dirname);
		iput(inode);
		return;
	}
